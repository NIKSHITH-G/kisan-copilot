...
5. ~~**Action**~~ DONE — LIVE VERIFIED 2026-07-20: Twilio creds in .env, WhatsApp sandbox joined from demo phone (+91807xxxx088), real Telugu advisory sent and status reached `read`. Sandbox join expires after ~72h idle — re-send the join code before demo day. `backend/actions.py` (Twilio REST via requests; WhatsApp sandbox default from-number) + `POST /send` + **MCP tool `send_advisory` (`backend/mcp_actions.py`) registered with CoCo as server `kisan-actions`** (`cortex mcp list`; config `~/.snowflake/cortex/mcp.json`) — CoCo can send the advisory itself: data + reasoning + action all CoCo-visible. Without `TWILIO_SID`/`TWILIO_TOKEN` in `.env`, every path returns `dry_run:true` with the exact would-send payload (tested via MCP stdio client AND REST). To go live: paste creds, join the Twilio WhatsApp sandbox from the target phone, re-test.
6. **Frontend** — BUILT 2026-07-20, redesigned same day to chat UI: `frontend/index.html` (single file, no build step) served at `/` — messaging-style thread, bottom composer with BOTH text input and tap-to-record mic (ScriptProcessor → WAV in browser), example prompts, stat tiles + validated price sparkline, in-app WhatsApp send via `/send`, EVIDENCE drawer. NO district field: ANSWER_FARMER extracts district/crop/language from the message itself (deterministic native-script crop words first, Cortex COMPLETE extraction for district/language; geocode-fail → Warangal fallback with `district_note`). `/ask_text` auto-detects language when `language_code` omitted. `/ask` path verified end-to-end; mic HUMAN-VERIFIED 2026-07-20 (live Telugu voice → correct hold + no-irrigation answer + EVIDENCE). NOTE: getUserMedia needs a secure context — works on http://localhost from the laptop; a phone over LAN http will BLOCK the mic → use an HTTPS tunnel (e.g. cloudflared) for phone demos.
